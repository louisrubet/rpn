...
class program : public stack {
 public:
  ...
  ret_value run(stack& stk, heap& hp) {
    ...
    // iterate commands
    for (int i = 0;
         (go_out == false) && (interrupt_now == false) && (i < (int)size());) {
    ...
    for (int i = 0; i < (int)size(); i++) {
      ...
      if (type == cmd_branch) {
        ...
        if (compare_branch(k, "if", 2)) {
          ...
          k->arg3 = vlayout[layout_index].index_if_or_do_or_while;
          ((branch*)seq_obj(vlayout[layout_index].index_then_or_unti_or_repeat))
              ->arg2 = next;  // fill branch2 (if was false) of 'then'
          ...
          k->arg1 = vstartindex[vstartindex.size() -
                                1];  // 'next' arg1 = 'start' index
          ...
          k->arg1 = vstartindex[vstartindex.size() -
                                1];  // fill 'step' branch1 = 'start' index
          ((branch*)seq_obj(vstartindex[vstartindex.size() - 1]))->arg2 =
              i;  // 'for' or 'start' arg2 = 'next' index
          vstartindex.pop_back();
          ...
          if (layout_index < 0) {
            ...
            if (vlayout[layout_index].is_do_unti) {
              ...
              ((branch*)seq_obj(
                   vlayout[layout_index].index_then_or_unti_or_repeat))
                  ->arg1 = i + 1;
              layout_index--;
            } else {
  ...
  ret_value show_error() {
    ...
    cerr << _err_context << ": error " << _err << ": "
         << s_ret_value_string[_err] << endl;
    switch (_err) {
      case ret_internal:
      case ret_deadly:
        ret = ret_deadly;
  ...
  static void apply_default() {
    ...
    number::s_mpfr_printf_format =
        string(MPFR_FORMAT_BEG) + ss.str() + string(MPFR_FORMAT_STD);
