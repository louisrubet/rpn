void rpn_plus() {
  ...
  if (_stack->get_type(0) == cmd_string && _stack->get_type(1) == cmd_string) {
    ...
    ostring* str = (ostring*)_stack->allocate_back(
        left_str_size + right_str_size + 1 + sizeof(ostring), cmd_string);
    str->_len = left_str_size + right_str_size;

    strncpy(str->_value, ((ostring*)_calc_stack.get_obj(1))->_value,
            left_str_size);
    strncat(str->_value, ((ostring*)_calc_stack.get_obj(0))->_value,
            right_str_size);
    _calc_stack.pop_back();
    _calc_stack.pop_back();
  }
  // adding numbers
  else if (_stack->get_type(0) == cmd_number &&
           _stack->get_type(1) == cmd_number) {
    number* right = (number*)_stack->pop_back();
    number* left = (number*)_stack->back();
    CHECK_MPFR(mpfr_add(left->_value.mpfr, left->_value.mpfr,
                        right->_value.mpfr, floating_t::s_mpfr_rnd));
  }
  // adding complexes
    ...
    CHECK_MPFR(mpfr_add(left->im()->mpfr, left->im()->mpfr, right->im()->mpfr,
                        floating_t::s_mpfr_rnd));
  }
  // adding complex+number
    ...
    number* right = (number*)_stack->pop_back();
    complex* left = (complex*)_stack->back();
    CHECK_MPFR(mpfr_add(left->re()->mpfr, left->re()->mpfr, right->_value.mpfr,
                        floating_t::s_mpfr_rnd));
  }
...
void rpn_minus() {
  ...
  if (_stack->get_type(0) == cmd_number && _stack->get_type(1) == cmd_number) {
    number* right = (number*)_stack->pop_back();
    number* left = (number*)_stack->back();
    CHECK_MPFR(mpfr_sub(left->_value.mpfr, left->_value.mpfr,
                        right->_value.mpfr, floating_t::s_mpfr_rnd));
  }
  // substracting complexes
    ...
    complex* right = (complex*)_stack->pop_back();
    complex* left = (complex*)_stack->back();
    CHECK_MPFR(mpfr_sub(left->re()->mpfr, left->re()->mpfr, right->re()->mpfr,
                        floating_t::s_mpfr_rnd));
  ...
  // substracting complex-number
    ...
    number* right = (number*)_stack->pop_back();
    complex* left = (complex*)_stack->back();
    CHECK_MPFR(mpfr_sub(left->re()->mpfr, left->re()->mpfr, right->_value.mpfr,
                        floating_t::s_mpfr_rnd));
  }
  // substracting number-complex
    ...
    number* right = (number*)_stack->pop_back();
    complex* left = (complex*)_stack->back();
    CHECK_MPFR(mpfr_sub(left->re()->mpfr, right->_value.mpfr, left->re()->mpfr,
                        floating_t::s_mpfr_rnd));
  } else
    ERR_CONTEXT(ret_bad_operand_type);
}
...
void rpn_mul() {
  ...
  if (_stack->get_type(0) == cmd_number && _stack->get_type(1) == cmd_number) {
    number* right = (number*)_stack->pop_back();
    number* left = (number*)_stack->back();
    CHECK_MPFR(mpfr_mul(left->_value.mpfr, left->_value.mpfr,
                        right->_value.mpfr, floating_t::s_mpfr_rnd));
  }
  // multiplying complexes (a+ib)*(x+iy)=(ax-by)+i(ay+bx)=a(x+iy)+b(-y+ix)
    ...
    // left: (a+ib)->(ax+iay)
    CHECK_MPFR(mpfr_mul(left->re()->mpfr, left->re()->mpfr, right->re()->mpfr,
                        floating_t::s_mpfr_rnd));
    CHECK_MPFR(mpfr_mul(left->im()->mpfr, left_sav->re()->mpfr,
                        right->im()->mpfr, floating_t::s_mpfr_rnd));

    // right: (x+iy)-> (bx-iby)
    CHECK_MPFR(mpfr_mul(right->im()->mpfr, left_sav->im()->mpfr,
                        right->im()->mpfr, floating_t::s_mpfr_rnd));
    CHECK_MPFR(
        mpfr_neg(right->im()->mpfr, right->im()->mpfr, floating_t::s_mpfr_rnd));
    CHECK_MPFR(mpfr_mul(right->re()->mpfr, left_sav->im()->mpfr,
                        right->re()->mpfr, floating_t::s_mpfr_rnd));

    // left=left+transpose(right)
    CHECK_MPFR(mpfr_add(left->re()->mpfr, left->re()->mpfr, right->im()->mpfr,
                        floating_t::s_mpfr_rnd));
    CHECK_MPFR(mpfr_add(left->im()->mpfr, left->im()->mpfr, right->re()->mpfr,
                        floating_t::s_mpfr_rnd));
  ...
  // multiplying complex*number
    ...
    number* right = (number*)_stack->pop_back();
    complex* left = (complex*)_stack->back();
    CHECK_MPFR(mpfr_mul(left->re()->mpfr, left->re()->mpfr, right->_value.mpfr,
                        floating_t::s_mpfr_rnd));
    CHECK_MPFR(mpfr_mul(left->im()->mpfr, left->im()->mpfr, right->_value.mpfr,
                        floating_t::s_mpfr_rnd));
  }
...
void do_divide_complexes() {
  ...
  // 2. left.re=ax+by
  CHECK_MPFR(mpfr_mul(left->re()->mpfr, left_sav->re()->mpfr,
                      right_sav->re()->mpfr,
  ...
  // 3. left.im=bx-ay
  CHECK_MPFR(mpfr_mul(left->im()->mpfr, left_sav->im()->mpfr,
                      right_sav->re()->mpfr,
                      floating_t::s_mpfr_rnd));  // left.im=bx
  CHECK_MPFR(mpfr_mul(right->im()->mpfr, left_sav->re()->mpfr,
                      right_sav->im()->mpfr,
  ...
  CHECK_MPFR(mpfr_div(left->re()->mpfr, left->re()->mpfr, ex->_value.mpfr,
                      floating_t::s_mpfr_rnd));
  CHECK_MPFR(mpfr_div(left->im()->mpfr, left->im()->mpfr, ex->_value.mpfr,
                      floating_t::s_mpfr_rnd));
...
void rpn_div() {
  ...
  if (_stack->get_type(0) == cmd_number && _stack->get_type(1) == cmd_number) {
    number* right = (number*)_stack->pop_back();
    number* left = (number*)_stack->back();
    CHECK_MPFR(mpfr_div(left->_value.mpfr, left->_value.mpfr,
                        right->_value.mpfr, floating_t::s_mpfr_rnd));
  }
  // dividing complexes
  else if (_stack->get_type(0) == cmd_complex &&
  ...
  // dividing complex/number
    ...
    number* right = (number*)_stack->pop_back();
    complex* left = (complex*)_stack->back();
    CHECK_MPFR(mpfr_div(left->re()->mpfr, left->re()->mpfr, right->_value.mpfr,
                        floating_t::s_mpfr_rnd));
    CHECK_MPFR(mpfr_div(left->im()->mpfr, left->im()->mpfr, right->_value.mpfr,
                        floating_t::s_mpfr_rnd));
  }
  // dividing number/complex
    ...
    CHECK_MPFR(mpfr_set(new_cplx->re()->mpfr,
...
void rpn_neg() {
  ...
  if (_stack->get_type(0) == cmd_number) {
    number* left = (number*)_stack->back();
    CHECK_MPFR(
        mpfr_neg(left->_value.mpfr, left->_value.mpfr, floating_t::s_mpfr_rnd));
  } else if (_stack->get_type(0) == cmd_complex) {
    complex* left = (complex*)_stack->back();
    CHECK_MPFR(
        mpfr_neg(left->re()->mpfr, left->re()->mpfr, floating_t::s_mpfr_rnd));
    CHECK_MPFR(
        mpfr_neg(left->im()->mpfr, left->im()->mpfr, floating_t::s_mpfr_rnd));
  } else
    ERR_CONTEXT(ret_bad_operand_type);
}
...
void rpn_inv() {
  ...
  if (_stack->get_type(0) == cmd_number) {
    number* left = (number*)_stack->back();
    CHECK_MPFR(mpfr_si_div(left->_value.mpfr, 1L, left->_value.mpfr,
                           floating_t::s_mpfr_rnd));
  } else if (_stack->get_type(0) == cmd_complex) {
    // 1. duplicate
    rpn_dup();
...
void rpn_purcent() {
  ...
  CHECK_MPFR(mpfr_mul(left->_value.mpfr, left->_value.mpfr, right->_value.mpfr,
                      floating_t::s_mpfr_rnd));
  CHECK_MPFR(mpfr_div_si(left->_value.mpfr, left->_value.mpfr, 100L,
                         floating_t::s_mpfr_rnd));
}
...
void rpn_purcentCH() {
  ...
  CHECK_MPFR(mpfr_mul_si(right->_value.mpfr, right->_value.mpfr, 100L,
                         floating_t::s_mpfr_rnd));
  CHECK_MPFR(mpfr_div(left->_value.mpfr, right->_value.mpfr, left->_value.mpfr,
                      floating_t::s_mpfr_rnd));
}
...
void rpn_power() {
  ...
  if (_stack->get_type(1) == cmd_number) {
    ...
    if (mpfr_cmp_d(left->_value.mpfr, 0.0) >= 0) {
      CHECK_MPFR(mpfr_pow(left->_value.mpfr, left->_value.mpfr,
                          right->_value.mpfr, floating_t::s_mpfr_rnd));
      _stack->pop_back();
      done_on_real = true;
    } else {
      ...
      CHECK_MPFR(mpfr_set_d(((number*)_stack->back())->_value.mpfr, 0.0,
                            floating_t::s_mpfr_rnd));
      rpn_r2c();
  ...
  if (!done_on_real) {
    if (_stack->get_type(1) == cmd_complex) {
      ...
      CHECK_MPFR(mpfr_pow(cplx->re()->mpfr, cplx->re()->mpfr,
                          exponent->_value.mpfr, floating_t::s_mpfr_rnd));

      // new arg=arg*exponent
      CHECK_MPFR(mpfr_mul(cplx->im()->mpfr, cplx->im()->mpfr,
                          exponent->_value.mpfr, floating_t::s_mpfr_rnd));
...
void rpn_squareroot() {
  if (_stack->get_type(0) == cmd_number) {
    ...
    if (mpfr_cmp_d(left->_value.mpfr, 0.0) >= 0)
      CHECK_MPFR(mpfr_sqrt(left->_value.mpfr, left->_value.mpfr,
                           floating_t::s_mpfr_rnd));
    else {
      ...
      rpn_r2c();
      _stack->allocate_back(number::calc_size(), cmd_number);
      CHECK_MPFR(mpfr_set_d(((number*)_stack->back())->_value.mpfr, 0.5,
                            floating_t::s_mpfr_rnd));
      rpn_power();
    }
  } else if (_stack->get_type(0) == cmd_complex) {
...
void rpn_square() {
  ...
  if (_stack->get_type(0) == cmd_number) {
    number* left = (number*)_stack->back();
    CHECK_MPFR(
        mpfr_sqr(left->_value.mpfr, left->_value.mpfr, floating_t::s_mpfr_rnd));
  } else if (_stack->get_type(0) == cmd_complex) {
    rpn_dup();
    rpn_mul();
  } else
...
void rpn_modulo() {
  ...
  CHECK_MPFR(mpfr_fmod(left->_value.mpfr, left->_value.mpfr, right->_value.mpfr,
                       floating_t::s_mpfr_rnd));
}
...
void rpn_abs() {
  ...
  if (_stack->get_type(0) == cmd_number) {
    ...
    CHECK_MPFR(mpfr_mul(cplx->re()->mpfr, cplx->re()->mpfr, cplx->re()->mpfr,
                        floating_t::s_mpfr_rnd));
    CHECK_MPFR(mpfr_mul(cplx->im()->mpfr, cplx->im()->mpfr, cplx->im()->mpfr,
                        floating_t::s_mpfr_rnd));
    ...
    CHECK_MPFR(mpfr_set(module->_value.mpfr, cplx->re()->mpfr,
                        floating_t::s_mpfr_rnd));
    CHECK_MPFR(mpfr_add(module->_value.mpfr, module->_value.mpfr,
                        cplx->im()->mpfr, floating_t::s_mpfr_rnd));
    CHECK_MPFR(mpfr_sqrt(module->_value.mpfr, module->_value.mpfr,
                         floating_t::s_mpfr_rnd));
...
void rpn_base() {
  ...
  ARG_MUST_BE_OF_TYPE(0, cmd_number);
  ARG_MUST_BE_OF_TYPE(1, cmd_number);
  if (mpfr_cmp_d(((number*)_stack->back())->_value.mpfr, (double)BASE_MIN) >=
          0 &&
      mpfr_cmp_d(((number*)_stack->back())->_value.mpfr, (double)BASE_MAX) <=
          0) {
    int base = (int)mpfr_get_d(((number*)_stack->pop_back())->_value.mpfr,
                               floating_t::s_mpfr_rnd);
...
void rpn_fact() {
  ...
  CHECK_MPFR(
      mpfr_gamma(left->_value.mpfr, left->_value.mpfr, floating_t::s_mpfr_rnd));
}
  ...
  if (mpfr_number_p(left->_value.mpfr)) {
    if (mpfr_zero_p(left->_value.mpfr))
      ...
      while (mpfr_greaterequal_p(left->_value.mpfr, one->_value.mpfr))
        mpfr_div(left->_value.mpfr, left->_value.mpfr, ten->_value.mpfr,
                 floating_t::s_mpfr_rnd);

      one->_value = 0.1;
      while (mpfr_less_p(left->_value.mpfr, one->_value.mpfr))
        mpfr_mul(left->_value.mpfr, left->_value.mpfr, ten->_value.mpfr,
                 floating_t::s_mpfr_rnd);
...
void rpn_fp() {
  ...
  CHECK_MPFR(
      mpfr_frac(left->_value.mpfr, left->_value.mpfr, floating_t::s_mpfr_rnd));
}
...
void rpn_min() {
  ...
  CHECK_MPFR(mpfr_min(left->_value.mpfr, left->_value.mpfr, right->_value.mpfr,
                      floating_t::s_mpfr_rnd));
}
...
void rpn_max() {
  ...
  CHECK_MPFR(mpfr_max(left->_value.mpfr, left->_value.mpfr, right->_value.mpfr,
                      floating_t::s_mpfr_rnd));
}
