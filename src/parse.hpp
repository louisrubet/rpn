...
static ret_value parse(const char* entry, program& prog) {
  ...
  if (_cut(entry, entries)) {
    ...
    for (vector<string>::iterator it = entries.begin(); it != entries.end();
         it++) {
...
private:
static ret_value get_fn(const char* fn_name, program_fn_t& fn,
                        cmd_type_t& type) {
  unsigned int i = 0;
  while (s_keywords[i].type != cmd_max) {
    if (strncasecmp(fn_name, s_keywords[i].name, sizeof(s_keywords[i].name)) ==
        0) {
      fn = s_keywords[i].fn;
...
static bool get_keyword(const string& entry, program& prog,
                        string& remaining_entry) {
  program_fn_t fn;
...
static bool get_symbol(const string& entry, program& prog,
                       string& remaining_entry) {
  bool ret = false;
  ...
  if (entry_len >= 1 && entry[0] == '\'') {
    if (entry_len == 1) {
      ...
      naked_entry_len =
          entry[entry_len - 1] == '\'' ? (entry_len - 2) : (entry_len - 1);
      // total object length
...
static bool get_other(const string& entry, program& prog,
                      string& remaining_entry) {
  bool ret = false;
  int entry_len = entry.size();
  unsigned int obj_len;
  ...
  if (entry_len >= 1) {
    ...
    // entry length without prefix / postfix
    naked_entry_len =
        entry[entry_len - 1] == '\'' ? (entry_len - 1) : (entry_len);
    // total object length
    obj_len = sizeof(symbol) + naked_entry_len + 1;
...
static bool get_string(const string& entry, program& prog,
                       string& remaining_entry) {
  bool ret = false;
  unsigned int obj_len;
  int entry_len = entry.size();
...
static bool get_program(string& entry, program& prog, string& remaining_entry) {
  ...
  if (entry_len >= 2 && entry[0] == '<' && entry[1] == '<') {
    ...
    // entry length without prefix / postfix
    if (entry_len >= 4 && entry[entry_len - 1] == '>' &&
        entry[entry_len - 2] == '>')
...
static bool get_number(string& entry, program& prog, string& remaining_entry) {
  ...
  if (entry.size() > 0) {
    ...
    if (entry.find_first_of("+-0123456789.ni@", 0) == 0) {
      ...
      number* num =
          (number*)prog.allocate_back(number::calc_size(), cmd_number);

      int mpfr_ret = mpfr_strtofr(num->_value.mpfr, entry.c_str(), &endptr,
                                  base, MPFR_DEFAULT_RND);
...
static bool get_complex(const string& entry, program& prog,
                        string& remaining_entry) {
  char* endptr;
  bool ret = false;
  ...
  if (entry.size() > 0) {
    ...
    if (comma != string::npos) {
      ...
      if (re_str.find_first_of(" +-0123456789.ni@", 0) == 0) {
        ...
        int mpfr_ret = mpfr_strtofr(cplx->re()->mpfr, re_str.c_str(), &endptr,
                                    0, MPFR_DEFAULT_RND);
      ...
      if (ret == true && im_str.find_first_of(" +-0123456789.ni@", 0) == 0) {
        ret = false;
        int mpfr_ret = mpfr_strtofr(cplx->im()->mpfr, im_str.c_str(), &endptr,
                                    0, MPFR_DEFAULT_RND);
...
static bool _obj_from_string(string& entry, program& prog,
                             string& remaining_entry) {
  bool ret = false;
...
static void entry_completion_generator(const char* text,
